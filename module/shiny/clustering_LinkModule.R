LinkModule = function(tf_miRNA,tf_mRNA,miRNA_mRNA,mRNA_mRNA,miRNA,TF,mRNA,network_similarity){
  ...
  #   if(file.exists("cluster.result_LinkModule.txt")){
  #     file.remove("cluster.result_LinkModule.txt")
  #   }
  #   sink("cluster.result_LinkModule.txt",append = TRUE, split = FALSE)
  #   cat("NULL")
  #   sink()
  #   if(file.exists("targetgene.txt")){
  #     file.remove("targetgene.txt")
  #   }
  #   sink("targetgene.txt",append = TRUE, split = FALSE)
  #   cat("NULL")
  #   sink()
  #   result=NULL
  #   cat("   LinkModule don't have input\n")
  #   return(result)
  # }
  # tf_miRNA <-read.csv("tf_miRNA_BRCA.txt", header = F, sep = "\t", stringsAsFactors = F)
  # tf_mRNA <- read.csv("tf_mRNA_BRCA.txt", header = F, sep = "\t", stringsAsFactors = F)
  # mRNA_mRNA <- read.csv("mRNA_mRNA_BRCA.txt", header = F, sep = "\t", stringsAsFactors = F)
  # miRNA_mRNA <- read.csv("miRNA_mRNA_BRCA.txt", header = F, sep = "\t", stringsAsFactors = F)
  # 
  ...
  for(i in 1:5){
    NRvec1 <- read.csv(vec_filename[[i]], header = F, sep = " ", stringsAsFactors = F)
    NRvec1 <- NRvec1[-1,]
    rownames(NRvec1) <- NRvec1[,1]
    NRvec1 <- NRvec1[,-c(1,ncol(NRvec1))]
    if(i==1){
      NRvec2 <- NRvec1
    } else {
      NRvec2 <- NRvec2 + NRvec1
    }
  }
  NRvec <- NRvec2/5
  write.table(NRvec, file = "NRvec_DBSCAN_avg.txt", sep = " ", row.names = TRUE,
              col.names = FALSE, quote = FALSE)
  

  NRvec <- read.csv("NRvec_DBSCAN_avg.txt", header = F,row.names = 1, sep = " ", stringsAsFactors = F)
  gene_names<-unique(c(rownames(NRvec),network[,1],network[,2]))
  a<-which(rownames(network_similarity)%in%gene_names)
  similarity <- network_similarity[a,]
  similarity <- similarity[,a]
  NRvec <- NRvec[rownames(similarity),]
  NRvec1 <- as.data.frame(scale(NRvec))
  ...
  ws <- 0.5
  feature <- ws*similarity1 + (1-ws)*NRvec1
  # feature <- cbind(similarity1,NRvec1)
  ...
  # if(file.exists("density_DBSCAN_reducing.txt")){
  #   file.remove("density_DBSCAN_reducing.txt")
  # }
  knns <- 5
  ...
  for(minpt in 5){
    for(eps in 7){
      ...
      # sink("density_DBSCAN_reducing.txt",append = TRUE, split = FALSE)
      # cat("minpt = ",minpt,"eps = ",eps,"avg_density = ", mean(cludensity),"\n")
      # sink()
    }
  }
  ...
  #     mRNA_name <- paste(mRNA_name," ",mRNA_clu[[i]][j],sep = "")
  #   }
  #   targetgene[ro,1] <- mRNA_name
  #   ro <- ro+1
  # }
  cat("   LinkModule over\n")
  return(result)
  
  # if(file.exists("cluster.result_DBSCAN_reducing.txt")){
  #   file.remove("cluster.result_DBSCAN_reducing.txt")
  # }
  # names(last_cluster) <- paste("M",1:length(last_cluster),sep="")
  # sink("cluster.result_DBSCAN_reducing.txt",append = TRUE, split = FALSE)
  # for(i in 1:length(last_cluster)){
  #   cat(names(last_cluster)[i],"  density = ",cludensity[i],"\n")
  #   cat("TF    ",length(TF_clu[[i]]), "    ",TF_clu[[i]],"\n")
  #   cat("miRNA    ",length(miRNA_clu[[i]]), "    ",miRNA_clu[[i]],"\n")
  ...
  # sink()
  # 
  # if(file.exists("NRL_targetgene_DBSCAN_reducing.txt")){
  #   file.remove("NRL_targetgene_DBSCAN_reducing.txt")
  # }
  # sink("NRL_targetgene_DBSCAN_reducing.txt",append = TRUE, split = FALSE)
  # for(i in 1:length(last_cluster)){
  #   cat(mRNA_clu[[i]],"\n")
  # }
  # sink()
